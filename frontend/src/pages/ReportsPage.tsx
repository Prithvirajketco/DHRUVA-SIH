    ...
    <div className="p-8 h-full overflow-y-auto bg-[#F4F5F7] text-slate-700 relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <button 
          ...
          className="flex items-center gap-2 bg-[#E11D48] hover:bg-rose-700 text-white px-4 py-2 rounded-lg font-bold transition-colors shadow-sm"
        >
          ...
          <tbody className="divide-y divide-slate-100">
            {reports.map(report => (
              <tr key={report.id} className="hover:bg-slate-50 transition-colors">
                <td className="p-4 font-medium text-[#1E293B] flex items-center gap-2">
                ...
                <td className="p-4 text-slate-600 text-sm max-w-xs truncate">{report.description}</td>
                <td className="p-4 text-slate-500 text-sm font-mono">{report.timestamp}</td>
                <td className="p-4">
      ...
      {showModal && (
        <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl p-6 w-full max-w-lg shadow-2xl border border-slate-200">
            <h2 className="text-2xl font-bold text-[#1E293B] mb-6">Report Incident</h2>
            <form onSubmit={handleReportSubmit} className="flex flex-col gap-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Location</label>
                <input required name="location" type="text" placeholder="e.g. Haflong NH54" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-[#1E293B] focus:outline-none focus:border-accent" />
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Incident Type</label>
                <select name="type" className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-[#1E293B] focus:outline-none focus:border-accent">
              ...
              <div>
                ...
                <textarea required name="description" rows={3} placeholder="Describe the severity..." className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-[#1E293B] focus:outline-none focus:border-accent"></textarea>
              </div>
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Photo (Optional)</label>
                <div className="w-full bg-slate-50 border border-dashed border-slate-300 rounded-lg p-6 flex flex-col items-center justify-center text-slate-500 cursor-pointer hover:bg-slate-100 transition-colors">
              ...
              <div className="flex gap-3 mt-4">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3 bg-white hover:bg-slate-50 text-slate-700 font-bold rounded-lg transition-colors border border-slate-300 shadow-sm">
                  Cancel
                </button>
                <button type="submit" className="flex-1 py-3 bg-[#E11D48] hover:bg-rose-700 text-white font-bold rounded-lg transition-colors shadow-sm">
                  Submit Report
                </button>
              </div>
            </form>
